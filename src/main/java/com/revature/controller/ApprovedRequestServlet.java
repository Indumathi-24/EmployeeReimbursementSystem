...
public class ApprovedRequestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<body>");
		out.println("<center>");
		out.println("<h1>Approved Requests of "+" "+email+"</h1>");
		EmployeeReimbursementService reimbursementRequest = new EmployeeReimbursementServiceImpl();
		List<EmployeeReimbursementEntity> approvedList =reimbursementRequest.viewApprovedRequests(email);
		System.out.println(approvedList);
		for(EmployeeReimbursementEntity r: approvedList)
		{
		...
		for(EmployeeReimbursementEntity elist :approvedList)
		{
